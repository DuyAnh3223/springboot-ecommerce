...
import { Package, Sliders, Images, Star } from "lucide-react";
...
export function StepSkuMatrix({
  ...
  const getPrimaryImagePreview = (galleryItems: SkuGalleryItem[] = []) => {
    if (!galleryItems || galleryItems.length === 0) return null;
    const primary = galleryItems.find((it) => it.isPrimary) || galleryItems[0];
    ...
    <Card className="border-none shadow-sm bg-white overflow-hidden">
      <CardHeader className="bg-slate-50/50 border-b border-slate-100">
        ...
        <CardDescription className="text-xs text-slate-450">
          Thiết lập chi tiết về mã định danh SKU, giá bán, tồn kho và bộ sưu tập hình ảnh riêng biệt cho từng biến thể sản phẩm.
        </CardDescription>
      </CardHeader>
      <CardContent className="p-6 space-y-6">
              ...
              <Table>
                <TableHeader className="bg-slate-50">
                  <TableRow>
                    ...
                    <TableHead className="text-slate-600 font-bold text-2xs py-3 w-1/4">Bộ sưu tập ảnh SKU</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {skuFields.map((field, index) => {
                    const galleryItems: SkuGalleryItem[] = field.galleryItems || [];
                    const primaryPreview = getPrimaryImagePreview(galleryItems);
                        ...
                        <TableCell className="py-2.5">
                          <div className="flex items-center gap-2">
                            <Button
                              ...
                              className="h-8 border-slate-200 text-slate-700 hover:bg-slate-50 text-xs font-semibold gap-1.5"
                            >
                              <Images className="size-3.5 text-shop_dark_green" />
                            ...
                            {primaryPreview && (
                              <div className="relative group">
                                <img
                                  src={primaryPreview}
                                  alt="Primary SKU Preview"
                                  className="size-8 rounded border border-slate-200 object-cover bg-white"
                                />
                                <span className="absolute -top-1 -right-1 bg-shop_dark_green text-white p-0.5 rounded-full shadow-2xs">
        ...
        {/* SKU Gallery Dialog */}
        {activeGalleryIndex !== null && skuFields[activeGalleryIndex] && (
          <SkuGalleryDialog
            open={activeGalleryIndex !== null}
            onOpenChange={(open) => {
              if (!open) setActiveGalleryIndex(null);
            }}
            skuTitle={skuFields[activeGalleryIndex].sku || `SKU #${activeGalleryIndex + 1}`}
            items={skuFields[activeGalleryIndex].galleryItems || []}
            onChange={(items) => {
              updateSkuField(activeGalleryIndex, {
                ...skuFields[activeGalleryIndex],
                galleryItems: items,
                isGalleryDirty: true,
              });
            }}
