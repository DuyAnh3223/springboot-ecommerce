...
import { categorySchema, CategoryFormValues } from "@/features/categories/schemas/category.schema";
...
export function useCategoryFormDialog({
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > MAX_FILE_SIZE) {
      setUploadError("Dung lÆ°á»£ng file tá»‘i Ä‘a lÃ  5MB. Vui lÃ²ng chá»n áº£nh nhá» hÆ¡n.");
      e.target.value = "";
      return;
    }

    if (!ALLOWED_TYPES.includes(file.type)) {
      setUploadError("Äá»‹nh dáº¡ng file khÃ´ng Ä‘Æ°á»£c há»— trá»£ (chá»‰ nháº­n JPG, PNG, WEBP, GIF).");
      e.target.value = "";
      return;
    }
    ...
    if (uploadMode === "file" && selectedFile) {
      try {
        ...
        if (uploadResult.error || !uploadResult.fileData) {
          setError(uploadResult.error || "Táº£i áº£nh lÃªn AWS S3 tháº¥t báº¡i.");
          return;
        }

        resolvedThumbnail = uploadResult.fileData.fileUrl;
      } catch (err: any) {
        setError("CÃ³ lá»—i xáº£y ra khi táº£i áº£nh lÃªn AWS S3.");
        return;
      }
    } else if (removeImage) {
