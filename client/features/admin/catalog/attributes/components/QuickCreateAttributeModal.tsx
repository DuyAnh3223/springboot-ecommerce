...
import { createAttributeAction } from "@/features/attributes/actions";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export default function QuickCreateAttributeModal({
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) {
      setError("TÃªn thuá»™c tÃ­nh khÃ´ng Ä‘Æ°á»£c Ä‘á»ƒ trá»‘ng.");
      return;
    }
      ...
      <DialogContent className="sm:max-w-[420px]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-1.5">
            <Sliders className="size-4.5 text-shop_light_green" /> Táº¡o nhanh thuá»™c tÃ­nh
          </DialogTitle>
          <DialogDescription>
            Äá»‹nh nghÄ©a má»™t thuá»™c tÃ­nh lÃµi má»›i trá»±c tiáº¿p vÃ o kho Master Data.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSave} className="space-y-4 py-1">
          ...
          <div className="space-y-1">
            <label className="text-xs font-bold text-slate-700">
              TÃªn thuá»™c tÃ­nh <span className="text-destructive">*</span>
            </label>
            <Input
              placeholder="VÃ­ dá»¥: GPU"
              value={name}
          ...
          <div className="space-y-1">
            <label className="text-xs font-bold text-slate-700">
              Kiá»ƒu dá»¯ liá»‡u <span className="text-destructive">*</span>
            </label>
            <select
              value={dataType}
              onChange={(e) => {
          ...
          {dataType === "ENUM" && (
            <div className="space-y-1 animate-in slide-in-from-top-1 duration-150">
              <label className="text-xs font-bold text-slate-700">
                GiÃ¡ trá»‹ danh sÃ¡ch (GÃµ Enter hoáº·c pháº©y Ä‘á»ƒ thÃªm)
              </label>
